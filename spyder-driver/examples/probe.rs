...
use spyder_driver::{Opcode, Spyder};
use std::time::Duration;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut spyder = Spyder::open_any()?;
    println!("opened device, USB PID = 0x{:04x}", spyder.pid());
    println!("(family: {})", if spyder.is_spyder_2024() { "Spyder 2024" } else { "SpyderX2" });

    // 0xC2 — combined info (HW version + serial + extended capabilities)
    println!("\n--- 0xC2 (get device info) ---");
    let raw = spyder.command(Opcode::GetInfo, &[], 0x25, false, Duration::from_secs(5))?;
    println!("raw 37-byte reply: {}", hex(&raw));
    let info = spyder.get_info()?;
    println!("hw version:     {}.{:02}", info.hw_version.0, info.hw_version.1);
    ...
    match spyder.command(Opcode::GetCalibration, &[0], 0x6C, true, Duration::from_secs(5)) {
