...
fn print_usage() {
    ...
    eprintln!("          unmanaged                      no description, compositor default");
    eprintln!("          srgb                           sRGB TF + sRGB primaries");
    eprintln!("          srgb-p3                        sRGB TF + Display-P3 primaries");
    eprintln!("          pq-bt2020[:peak=,maxcll=,maxfall=,min=]   PQ + BT.2020");
    eprintln!("          pq-p3[:...]                    PQ + Display-P3");
    eprintln!("          scrgb                          Windows-scRGB (1.0 = 80 cd/m²)");
    eprintln!("        the buffer format is chosen per representation from what the");
    eprintln!("        compositor advertises (fp16 preferred for HDR, float required for");
    eprintln!("        scrgb, 8-bit for SDR); pin one with buf=<wl_shm name>, e.g.");
    eprintln!("        pq-bt2020:buf=xrgb2101010 to probe PQ through a 10-bit buffer.");
